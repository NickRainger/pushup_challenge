<template>
  <div class="flex flex-col gap-2 bg-base-200 p-4 rounded-xl">
    ...
    <div v-for="groupUser, i in  store.groupUsers " :key="groupUser.id" class="text-xl font-bold flex items-center gap-2">
      ...
      <div v-if="i == 0 && canShow(<ExtendedGroupUser>groupUser)" class="text-3xl material-symbols-rounded fill"
        :class="{ 'text-amber-400': (groupUser.totalReps || 0) >= 100 }">
        star
      </div>

      <div v-else-if="i == 1 && canShow(<ExtendedGroupUser>groupUser)"
        class=" text-3xl text-[#898d90] material-symbols-rounded fill">star</div>
      <div v-else-if="i == 2 && canShow(<ExtendedGroupUser>groupUser)"
        class=" text-3xl text-[#b06c28] material-symbols-rounded fill">star</div>
